...
TOKEN = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJlbWFpbCI6ImdsZW5kYS5tZXJtYUB0ZWNzdXAuZWR1LnBlIn0.BP29JgrT5a0jiopCv7Lg7Jj0LZSFe7Ms4nacVdisleU'
        ...
        for partner in self:
            ...
            if partner.tipo_doc_id.code in ('1','6') and partner.vat is not False:
                ...
                if tipo_doc:
                    ...
                    if not respuesta:
                        raise Warning("Server not reponding now")
                    if not respuesta[0]:
                        raise Warning(respuesta[1])
                    try:
                        respuesta_json = json.loads(respuesta[1].decode())
                    except ValueError as e:
                        raise Warning(("Server response content...: %s" % e))
                    if tipo_doc == 'ruc':
